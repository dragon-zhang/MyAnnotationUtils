...
import org.springframework.core.annotation.MyMergedAnnotations.SearchStrategy;
...
public abstract class MyAnnotatedElementUtils {
	...
	public static MultiValueMap<String, Object> getAllAnnotationAttributes(AnnotatedElement element,
		...
		return getAnnotations(element).stream(annotationName)
				...
				.collect(MergedAnnotationCollectors.toMultiValueMap(MyAnnotatedElementUtils::nullIfEmpty, adaptations));
	}
	...
	private static MyMergedAnnotations getAnnotations(AnnotatedElement element) {
		return MyMergedAnnotations.from(element, SearchStrategy.INHERITED_ANNOTATIONS, RepeatableContainers.none());
	}

	private static MyMergedAnnotations getRepeatableAnnotations(AnnotatedElement element,
		...
		return MyMergedAnnotations.from(element, SearchStrategy.INHERITED_ANNOTATIONS, repeatableContainers);
	}

	private static MyMergedAnnotations getMultipleAliasesAnnotations(AnnotatedElement element) {
		return MyMergedAnnotations.fromMultipleAliasesAnnotations(element, SearchStrategy.INHERITED_ANNOTATIONS, RepeatableContainers.none());
	}

	private static MyMergedAnnotations findAnnotations(AnnotatedElement element) {
		return MyMergedAnnotations.from(element, SearchStrategy.TYPE_HIERARCHY, RepeatableContainers.none());
	}

	private static MyMergedAnnotations findRepeatableAnnotations(AnnotatedElement element,
																 @Nullable Class<? extends Annotation> containerType, Class<? extends Annotation> annotationType) {

		RepeatableContainers repeatableContainers = RepeatableContainers.of(annotationType, containerType);
		return MyMergedAnnotations.from(element, SearchStrategy.TYPE_HIERARCHY, repeatableContainers);
	}
