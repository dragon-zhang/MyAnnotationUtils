...
final class MyMultipleAliasAnnotations implements MyMergedAnnotations {
	 ...
	 */
	static final MyMultipleAliasAnnotations NONE = new MyMultipleAliasAnnotations(
	...
	private MyMultipleAliasAnnotations(@Nullable Object source, Annotation[] annotations,
									   RepeatableContainers repeatableContainers, AnnotationFilter annotationFilter) {
		this.source = source;
		this.element = null;
	...
	public <A extends Annotation> MergedAnnotation<A> get(Class<A> annotationType,
		...
		MergedAnnotation<A> result = scan(annotationType,
				new MergedMultipleAliasAnnotationFinder<>(annotationType, predicate, selector));
	...
	static MyMergedAnnotations from(@Nullable Object source, Annotation[] annotations,
		...
		return new MyMultipleAliasAnnotations(source, annotations, repeatableContainers, annotationFilter);
	}

	private static boolean isMappingForType(MyAnnotationTypeMapping mapping,
			...
			for (Annotation annotation : annotations) {
				if (annotation != null) {
					...
					if (type != null && !this.annotationFilter.matches(type)) {
						...
						if (!this.directOnly) {
							MyAnnotationTypeMappings mappings = MyAnnotationTypeMappings.forAnnotationType(type);
	...
	private class MergedMultipleAliasAnnotationFinder<A extends Annotation>
		...
		MergedMultipleAliasAnnotationFinder(Object requiredType, @Nullable Predicate<? super MergedAnnotation<A>> predicate,
											@Nullable MergedAnnotationSelector<A> selector) {
		...
		/**
		 * the only change from {@link TypeMappedAnnotations.MergedAnnotationFinder}
		 */
		@Nullable
		private MergedAnnotation<A> process(
			...
			MyAnnotationTypeMappings mappings = MyAnnotationTypeMappings.forMultipleAnnotationType(
	...
	private static class Aggregate {
		...
		Aggregate(int aggregateIndex, @Nullable Object source, List<Annotation> annotations) {
			...
			for (int i = 0; i < annotations.size(); i++) {
				this.mappings[i] = MyAnnotationTypeMappings.forAnnotationType(annotations.get(i).annotationType());
			}
		}
